...
class Kasbi:
    ...
    def Print_string(self, href=0,string=''):
        ...
        result = []
        result.append(href)
        for x in string: result.append(ord(x))
        if len(result)<41:
            for i in range(41-len(result)):result.append(ord(' '.encode('cp866')))
    ...
    def Sale(self,price,count,tgroup=1,sec=1,discount=0,rate_discount=0,summ_rate_disc=0,tax0=0,tax1=0,tax2=0,tax3=0,string='',corr=0):
        ...
        data=[corr]
        for x in array.array('B',count).tostring():data.append(x)
        for x in array.array('B',price).tostring():data.append(x)
        ...
        for x in string:data.append(ord(x))
        if len(string)<37:
            for i in range(37-len(string)):data.append(ord(' '.encode('cp866')))
    ...
    def closeChek(self,summ0=0,summ1=0,summ2=0,summ3=0,discount=0,rate_discount=0,summ_rate_disc=0,corr=0):
        ...
        rate_discount=pack('h',self.float2int(rate_discount,2))
        summ_rate_disc=pack('l',self.float2int(summ_rate_disc,2))
        for x in array.array('B',summ).tostring():data.append(x)
